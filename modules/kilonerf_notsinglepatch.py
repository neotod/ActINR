...
def multibias(im_list, nscales, stopping_mse, config, pretrained_models=None, temporal_weightshare=True,test_data=None):
    ...
    tbar = tqdm.tqdm(range(config.epochs))
    rec_t = torch.zeros_like(imten)

    for idx in tbar:
        lr = config.lr*pow(0.1, idx/config.epochs)
        optim.param_groups[0]['lr'] = lr
        indices_t = torch.randperm(nimg)
        for t_idx in range(0,nimg,6): 
            
            t_indices = indices_t[t_idx:min(nimg,t_idx+6)]
            ...
            if config.warm_start and idx < config.warm_epochs:
                ...
                #im_estim = fold(im_out).reshape(split_size, -1, H, W)
                loss = criterion(im_estim, imten[t_indices,...])
            
            with torch.no_grad():
                rec_t[t_indices,...] = im_estim
    ...
    rec_test = torch.zeros_like(imtest)
    ...
    with torch.no_grad():
        ...
        if config.slowmo:
            ...
            for t_idx in range(0,nimg_test,2): 
                ...
                im_estim = fold(im_out).reshape(split_size, -1, H, W) 
                with torch.no_grad():
                    rec_test[t_indices,...] = im_estim

        if config.slowmo:
            im_estim = fold(im_out).reshape(t_len, -1, H, W)
        best_img =  rec_test.detach().permute(0, 2, 3, 1).cpu().numpy()
        if not config.slowmo:
            mse_list_test = ((rec_test - imtest)**2).mean([1, 2, 3])
