...
from big_unfold import unfoldNd
from fold_utils import _get_kernel_size_numel, _get_numel_from_shape, _tuple
...
def foldNd(input, output_size, kernel_size, dilation=1, padding=0, stride=1):
    ...
    if output_size_numel > pow(2, 24):
        dtype = torch.float64
    else:
        dtype = torch.float32
    idx = torch.arange(output_size_numel, dtype=dtype, device=device).reshape(
        1, 1, *output_size
    )
