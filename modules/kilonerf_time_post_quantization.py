    ...
    if rank==0:
        ...
        with torch.no_grad():
            ...
            if not config.qat:
                ...
                n = float(2 ** config.quant_model_bit - 1)
                for k,v in cur_ckt.items():
                    weight = torch.tanh(v)
                    max_w = torch.max(torch.abs(weight)).detach()
                    weight = weight / 2 / max_w + 0.5
                    quant_v = torch.round(weight * n) 
                    #quant_v = qfn.apply(weight, config.quant_model_bit)
                    valid_quant_v = quant_v
                    quant_v_list.append(valid_quant_v.flatten())
                cat_param = torch.cat(quant_v_list)
                quant_v_list = cat_param.tolist()
