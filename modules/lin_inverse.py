...
class RadonFunc(nn.Module):
    def __init__(self, angles, is_3d=False):
        super().__init__()
        self.angles = angles
        self.is_3d = is_3d

    def forward(self, x):
        return radon(x[None, ...], self.angles, self.is_3d)
...
def radon(imten, angles, is_3d=False):
    ...
    if is_3d:
        sinogram = imten_rot.mean(2).squeeze().permute(1, 0, 2)
    else:
        sinogram = imten_rot.mean(2).squeeze()
