...
def multibias(im_list, nscales, stopping_mse, config, pretrained_models=None, temporal_weightshare=True,test_data=None):
    ...
    def getGradient(name):
        def backward_hook(module, grad_input, grad_output):
            ...
            #print(f'Gradients size: {gradients['grad'][0].size()}') 
    ...
    #imten_list = [torch.tensor(im)[None, ...] for im in im_list]
    imten_list = [cv2.resize(im,None,fx=1/2,fy=1/2,interpolation=cv2.INTER_AREA) for im in im_list]

    tau=3e1
    noise_snr=1
    #imten = torch.cat(imten_list, 0)
    # resize
    #imten = torch.nn.functional.interpolate(imten,size=(240,480),mode="area")

    imten_noise_list = []

    for im_ in imten_list:
        im_noise= measure(im_,noise_snr=noise_snr,tau=tau)
        imten_noise_list.append(torch.tensor(im_noise).permute(2,0,1))

    imten_noise = torch.stack(imten_noise_list,0).cuda()
    imten = [torch.tensor(imt).permute(2,0,1) for imt in imten_list]
    imten = torch.stack(imten,0).cuda()
    imten = imten.cuda()

    
    t_idx_train = torch.tensor(config.train_idx,device="cuda",dtype=torch.float)
    #t_idx_train = torch.arange(0,config.n_frames,1,device='cuda',dtype=torch.float)
    n_frames_ = config.train_idx[-1]
    t_idx_train = ((t_idx_train - (n_frames_)/2)/((n_frames_)/2))
    #imten_der = torch.gradient(imten,dim=0,spacing=(t_idx_train,))[0]

    B, C, H, W = imten.shape

    print('Input PSNR: %.2f dB'%utils.psnr(imten, imten_noise))
    #imten_der = imten_der.flatten(2,3)
    
    imten = imten.flatten(2,3)
    imten_noise = imten_noise.flatten(2,3)
    ...
    #    print(key, model_state_dict[key].dtype)
    if config.inference:
        for params_ in model.parameters():
    ...
    if config.slowmo:
        t_idx_test = torch.arange(0,config.n_frames,0.1,device='cuda',dtype=torch.float)
    else:
        t_idx_test = torch.tensor(config.test_idx,device='cuda',dtype=torch.float)
        #t_idx_test = torch.arange(0.5,config.n_frames-0.5,1,device='cuda',dtype=torch.float)
    ...
    # imtest = imten
    # nimg_test = nimg
    # t_idx_test = t_idx_train
    
    #backward_hook=model.net[1].linear.register_full_backward_hook(getGradient("layer1"),prepend=False)
    mse_array = np.zeros(config.epochs)
    best_mse = float('inf')
    best_img = None
    maxpoints = 230*230
    ...
    for idx in tbar:
        ...
        for b_idx in range(0,H*W,maxpoints):
            ...
            b_coords = coords_chunked[:, b_indices, ...].cuda()
            ...
            im_out = model(b_coords, learn_indices,t_idx_train)
            im_out = im_out.permute(0, 3, 2, 1).squeeze(-1)
            
            with torch.no_grad():
                rec[:,:,b_indices] = im_out


            #im_estim = fold(im_out).reshape(nimg, -1, H, W)
            
            if config.warm_start and idx < config.warm_epochs:
                loss = criterion(im_estim[0], imten[0])
            else:  
                # Boundary condition first order derivative loss
                loss = ((im_out -(imten_noise[:,:,b_indices]))**2).mean()
                #loss += 0.01*(((jvp_out - imten_der[:,:,b_indices])**2).mean())
                #loss = criterion(im_estim, imten)
    ...
    # Do a nuclear norm prox gradient step here
        ...
        with torch.no_grad():
            mse_list = list(((imten -rec)**2).mean([1,2]).cpu().numpy())
            lossval = sum(mse_list) / len(mse_list)
            im_estim_cpu = rec.reshape(-1,3,H,W).detach().permute(0,2,3,1).cpu().numpy()
        ...
        tbar.set_description(('%.3f, %.3f')%mse_minmax)
        tbar.refresh()
    ...
    imtest = imtest.flatten(2,3)
    rec_t = torch.zeros((t_idx_test.shape[1:2] + imtest.shape[1:])).to(imtest.device).to(imtest.dtype)
    maxpoints=50*50
    with torch.no_grad():
        if config.slowmo:
            t_idx_test_batch = torch.split(t_idx_test,50,dim=1)
            t_idx_test_rec = torch.split(torch.arange(t_idx_test.shape[1]),50)
            t_len = t_idx_test.shape[1]
            im_out_list = []
            for t_idx_rec,t_idx_inst in zip(t_idx_test_rec,t_idx_test_batch):
                indices = torch.arange(H*W)
                for b_idx in range(0,H*W,maxpoints):
                    b_indices = indices[b_idx:min(H*W, b_idx+maxpoints)]
                    b_coords = coords_chunked[:, b_indices, ...].cuda()
                    b_indices = b_indices.cuda()
                    im_out = model(b_coords,learn_indices,t_idx_inst)
                    im_out = im_out.permute(0, 3, 2, 1).squeeze(-1)
                    with torch.no_grad():
                        low = int(t_idx_rec[0])
                        high = int(t_idx_rec[-1]+1)
                        rec_t[low:high,:,b_indices] = im_out
            ...
            for b_idx in range(0,H*W,maxpoints):
                ...
                b_coords = coords_chunked[:, b_indices, ...].cuda()
                b_indices = b_indices.cuda()
                im_out = model(b_coords,learn_indices,t_idx_test)
                im_out = im_out.permute(0, 3, 2, 1).squeeze(-1)
                with torch.no_grad():
                    rec_t[:,:,b_indices] = im_out


        #im_out = im_out.permute(0, 3, 2, 1).flatten(1,2)
        if config.slowmo:
            im_estim = rec_t.reshape(-1,3,H,W).detach()
        else:
            im_estim = rec_t.reshape(-1,3,H,W).detach()
            #im_estim = fold(im_out).reshape(nimg_test, -1, H, W)
        best_img =  im_estim.detach().permute(0, 2, 3, 1).cpu().numpy()
        if not config.slowmo:
            mse_list_test = ((rec_t - imtest)**2).mean([1,2])
            mse_list_test = tuple(mse_list_test.cpu().numpy().tolist())
            psnr_array_test = -10*np.log10(np.array(mse_list_test))
            print('test psnr: {:.3f}'.format(np.average(psnr_array_test)))
        psnr_array_train = -10*np.log10(np.array(mse_list))

        print('train psnr: {:.3f}'.format(np.average(psnr_array_train)))
