...
class AdaptiveLinearWithChannel(nn.Module):
    ...
    def __init__(self, input_size, output_size, channel_size,
        ...
        #self.t_enc = PositionalEncoding('1.05_40',n_chunks=w_nchan)
        ...
        if bias:
            if add_dim:
                ...
                if dtype == torch.cfloat:
                    ...
                    bias = (0.1*torch.rand(config.n_frames,channel_size,1,output_size, dtype=torch.cfloat)-0.1).to(rank) # interpolation
                    #bias = (0.1*torch.rand(config.no_tiles,channel_size,1,10, dtype=torch.float)-0.1).to(rank) # interpolation
                    ...
                    #    bias = (0.1*torch.rand(config.no_tiles,channel_size,1,10, dtype=torch.float)-0.1).to(rank)  # interpolation
                    #else:
                    bias = (0.1*torch.rand(config.n_frames,channel_size,1,output_size, dtype=torch.float)-0.1).to(rank)
    ...
    def forward(self, x, indices,t=None, ex_biases=None,coords=None,epochs=None):        
        ...
        #     hyper_shift = self.hyper_shift(t,indices).permute(1, 0, 2).unsqueeze(-2) # Global Coordinates
        ...
        return  output + bias # hyper_shift  #+ bias # affine*(output +  hyper_shift) if self.output_size!=3 else   + bias 
...
class AdaptiveMultiWIRE(nn.Module):
    def __init__(self, in_features, hidden_features, 
        ...
        self.fold = torch.nn.Fold(output_size=(1, 300),
                kernel_size=[1,300],
                stride=[1,300])
    ...
    def forward(self, inp, indices,t=None, ex_biases=None,epochs=None,gt_data=None):            
        ...
        # img_idx = int(round(t[0].item()*31))  
        # if img_idx==0 or img_idx==31:
        #     if not os.path.exists("./total/"):
        #         os.makedirs("./total/")    
        #     soft_blue = '#4F81BD'
        #     muted_green = '#9BBB59'
        #     light_coral = '#F28A8C'
        #     output_l = output_ls[0].detach().clone()[...,4]
        #     output_c = output_ls[0].detach().clone()[...,7]
        #     output_r = output_ls[0].detach().clone()[...,8]

        #     prediction = output_ls[1].detach().clone().real.flatten().cpu().numpy()
        #     output_l = self.fold(output_l.permute(0,2,1))
        #     output_c = self.fold(output_c.permute(0,2,1))
        #     output_r = self.fold(output_r.permute(0,2,1))
        ...
        #     plt.plot(coords.detach().cpu().numpy()[0,0,:,0],output_l.real.flatten(),color=soft_blue,label="Wavelet Basis")
