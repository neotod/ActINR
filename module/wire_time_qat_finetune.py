...
from modules.siren_hyperinr_qat_finetune import AdaptiveMultiSiren
...
class AdaptiveLinearWithChannel(nn.Module):
    ...
    def __init__(self, input_size, output_size, channel_size,
        ...
        self.quantize_fn = QuantNoise(bitwidth=8,noise_ratio=0.0,ste=False,axis=compute_best_quant_axis(self.weight))
        if bias:
            if add_dim:
        ...
        if dtype == torch.cfloat:
            ...
            self.hyper_shift = hyper_network(in_features=80+10,
                        ...
                        hidden_omega_0=30,
                        n_img=n_img,
                        hyper=True,pos_encode=False,gating=input_size,scale=False,qbit=config.quant_model_bit
                        )
    ...
    def reset_parameters(self,weight,bias):
        ...
        for i in range(weight.shape[0]):
            if self.complex_layer:
                ...
                self.weight.data = torch.atanh(self.weight.data)
            else:
        ...
        for i in range(bias.shape[0]):
            if self.complex_layer:
                ...
                self.bias.data = torch.atanh(self.bias.data)
            else:
                fan_in , fan_out = torch.nn.init._calculate_fan_in_and_fan_out(weight[0,...])
                bound = np.sqrt(1/(fan_out))
                bias[i,...].uniform_(-bound,bound)
    ...
    def forward(self, x, indices,t=None, ex_biases=None,coords=None,epochs=None):        
        ...
        weight = self.quantize_fn(self.weight)
        #bias = self.quantize_fn(self.bias)
        # if self.output_size!=3:
...
class AdaptiveMultiWIRE(nn.Module):
    ...
    def forward(self, inp, indices,t=None, ex_biases=None,epochs=None,gt_data=None):            
        ...
        # img_idx = int(round(t[0].item()*51))  
        # if img_idx==0 or img_idx==51:
        #     if not os.path.exists("./total/"):
        #         os.makedirs("./total/")    
        #     soft_blue = '#4F81BD'
        #     muted_green = '#9BBB59'
        #     light_coral = '#F28A8C'
        #     output_l = output_ls[0].detach().clone()[...,2]
        #     #output_c = output_ls[0].detach().clone()[...,1]
        #     output_r = output_ls[0].detach().clone()[...,3]

        #     #prediction = output_ls[1].detach().clone().real.flatten().cpu().numpy()
        #     output_l = self.fold(output_l.real.permute(0,2,1))
        #     #output_c = self.fold(output_c.permute(0,2,1))
        #     output_r = self.fold(output_r.real.permute(0,2,1))
            
        #     output_l = output_l.squeeze(0).squeeze(0).cpu().numpy()
        #    #output_c = output_c.squeeze(0).squeeze(0).cpu().numpy() 
        #     output_r = output_r.squeeze(0).squeeze(0).cpu().numpy() 
        #     plt.imshow(output_l, cmap='gray')
        #     plt.axis("off") 
        #     plt.savefig("act_left_{}.svg".format(img_idx), format="svg", bbox_inches='tight')
        #     plt.imshow(output_r, cmap='gray')
        #     plt.axis("off") 
        #     plt.savefig("act_right{}.svg".format(img_idx), format="svg", bbox_inches='tight')

        #     plt.imshow(coords.detach().cpu().numpy()[0,0,:,0],output_l.real.flatten(),color=soft_blue,label="Wavelet Basis")
        #     plt.plot(coords.detach().cpu().numpy()[0,0,:,0],output_c.real.flatten(),color=soft_blue,label="")
        #     plt.plot(coords.detach().cpu().numpy()[0,0,:,0],output_r.real.flatten(),color=soft_blue,label="")
        #     plt.plot(coords.detach().cpu().numpy()[0,0,:,0],abs(gt_data.flatten().cpu().numpy() - prediction),color=light_coral,label="Absolute Error")
        #     plt.plot(coords.detach().cpu().numpy()[0,0,:,0],gt_data.flatten().cpu().numpy(),label="Ground Truth",color=muted_green)
        #     plt.legend(loc='upper left', bbox_to_anchor=(-0.05, 1.05), ncol=3, frameon=False)
        #     plt.axis('off')
        #     plt.savefig('./total/{}.png'.format(img_idx), bbox_inches='tight')
        #     plt.show()
        #     plt.close()
