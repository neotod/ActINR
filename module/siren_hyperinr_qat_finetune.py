...
class PosEncoding(nn.Module):
    ...
    def forward(self, coords):
        ...
        return torch.cat(pe_list, 1).reshape(tile_n,chunk_no,coord_no,self.out_dim)
...
class AdaptiveLinearWithChannelHyper(nn.Module):
    ...
    '''
    def __init__(self, input_size, output_size, channel_size,
                 share_weights=False,n_img=None,last=False,qbit=None):
    ...
    def reset_parameters(self, weights, bias):
        ...
        bound = 1 / math.sqrt(fan_in)
        torch.nn.init.uniform_(bias, -bound, bound)
        weights.data = torch.atanh(weights.data)
        bias.data = torch.atanh(bias.data)
    ...
    def forward(self, x, indices,t=None):
        ...
        return (torch.matmul(x, weight_q) + self.bias[indices, ...])
...
class AdaptiveMultiSineLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, n_channels,
        ...
        self.linear = AdaptiveLinearWithChannelHyper(in_features,
                                                out_features,
                                                n_channels,n_img=n_img,share_weights=share_weights)
...
class AdaptiveMultiReLULayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, n_channels, is_first=False, 
                 omega_0=30, share_weights=None, const=None,n_img=None,hyper=False,qbit=None):
        super().__init__()        
        self.in_features = in_features
        self.out_features = out_features
        self.linear = AdaptiveLinearWithChannelHyper(in_features,
                                                out_features,
                                                n_channels,n_img=n_img,share_weights=share_weights,qbit=qbit)
        #self.scale = 5
        self.relu = torch.nn.GELU()
        
        
    def forward(self, input, indices,t):
        #if self.out_features==25:
        #return torch.exp(-(self.scale * self.linear(input,indices,t))**2)
        #else:
        return self.relu(self.linear(input, indices,t))
    

class AdaptiveMultiTanhLayer(nn.Module):
    '''
        Implements ReLU activations with multiple channel input.
    ...
    def __init__(self, in_features, out_features, n_channels, is_first=False, 
                 omega_0=30, share_weights=None, const=None,n_img=None,hyper=False):
        super().__init__()        
        self.in_features = in_features
        self.out_features = out_features
        self.linear = AdaptiveLinearWithChannelHyper(in_features,
    ...
    def forward(self, input, indices,t):
        ...
        #return torch.exp(-(self.scale * self.linear(input,indices,t))**2)
        #else:
        return self.tanh(self.linear(input, indices,t))
...
class AdaptiveMultiSiren(nn.Module):
    def __init__(self, in_features, hidden_features, hidden_layers, 
                 ...
                 const=1.0,n_img=None,hyper=False,gating=None,scale=False,qbit=None):
        super().__init__()
        ...
        if nonlin == 'sine':
            ...
            self.nonlin = AdaptiveMultiTanhLayer
        self.n_img = n_img
        self.hyper= hyper
        self.pos_encode = pos_encode
        if self.pos_encode:
        ...
        self.net.append(self.nonlin(in_features, hidden_features, n_channels,
                                  ...
                                  const=const,n_img=n_img,hyper=hyper,qbit=qbit))
        ...
        if outermost_linear:
            ...
            final_linear = AdaptiveLinearWithChannelHyper(
                ...
                out_features,
                n_channels,
                share_weights=share_weights,n_img=n_img,last=True,qbit=qbit)
            ...
            with torch.no_grad():
                ...
                if nonlin=='sine':
                    ...
                    if scale:
                        final_linear.weight.data = math.sqrt(1/100)*final_linear.weight.data
                        final_linear.bias.data = math.sqrt(1/100)*final_linear.bias.data
